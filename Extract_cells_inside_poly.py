...
fileN='/users/nponomar/grids/icon_Zurich_1_DOM02_extractedcells_python.nc'
dataset = Dataset(fileN, maskandscale=True, mmap=False)
dataset = xr.load_dataset(fileN)
print(dataset)

lats = dataset.variables['vlat'][:]
lons = dataset.variables['vlon'][:]

clon1 = np.rad2deg( lons )
clat1 = np.rad2deg( lats )
...
plt.savefig('Extracted_wit_cdo_points.png')